...
### 第一阶段：静态代码分析 (Mermaid Flowchart)

```mermaid
graph TD
    A["1. 代码预处理<br>(美化、分块、索引)"] --> B["2. LLM辅助分析<br>(模式识别、函数解析、依赖映射、API追踪)"]
    B --> C["3. 交叉验证<br>(多轮迭代、一致性检查、源码对照)"]
```

### 第二阶段：动态行为验证 (Mermaid Flowchart)

```mermaid
graph TD
    D["1. 运行时分析<br>(函数调用追踪、状态变化监控、性能指标收集)"] --> E["2. 集成测试<br>(组件交互验证、边界条件测试、错误恢复验证)"]
```
...
#### 2. 工具执行框架 (Tool Execution Framework - Mermaid Flowchart)

```mermaid
graph TD
    T1[1. 工具发现和注册] --> T2[2. 参数验证和类型检查]
    T2 --> T3[3. 权限验证和安全检查]
    T3 --> T4[4. 资源分配和环境准备]
    T4 --> T5[5. 并发执行和状态监控]
    T5 --> T6[6. 结果收集和清理回收]
...
#### 4. 安全防护框架 (Security Framework - Mermaid Flowchart)

```mermaid
graph TD
    S1[1. UI输入验证层] --> S2[2. 消息路由验证层]
    S2 --> S3[3. 工具调用验证层]
    S3 --> S4[4. 参数内容验证层]
    S4 --> S5[5. 系统资源访问层]
    S5 --> S6[6. 输出内容过滤层]
